...
from games.Bomberman.node import Node
...
class Enemy:
    ...
    def create_grid_dijkstra(self, map, bombs, explosions, enemys):
        grid = [[None] * len(map[0]) for r in range(len(map))]
    ...
    def load_animations(self, en, scale):
        ...
        image_path = 'games\Bomberman\images\enemy\e'
        if en == '':
            image_path = 'games\Bomberman\images\hero'

        f1 = pygame.image.load(image_path + str(en) + 'f0.png')
        f2 = pygame.image.load(image_path + str(en) + 'f1.png')
        f3 = pygame.image.load(image_path + str(en) + 'f2.png')
